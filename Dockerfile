...
RUN echo "#!/usr/bin/with-contenv bash" > /etc/cont-init.d/bootstrap_container && \
    ...
    echo "echo \"alias run_beeline=/opt/dataworks/apache-hive-3.1.2-bin/bin/beeline -n ${USER} -p $JWT_TOKEN -u jdbc:hive2://${EMR_HOST_NAME}:10000/default;\" >> /etc/profile.d/rstudio.sh" >> /etc/cont-init.d/bootstrap_container && \
