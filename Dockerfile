...
ukbabynames BAMMtools ggwordcloud samplingbook labourR RSNNS brnn ggraph
...
RUN echo "#!/usr/bin/with-contenv bash" > /etc/cont-init.d/bootstrap_container && \
    ...
    echo "echo \"alias run_beeline=\"/opt/dataworks/apache-hive-3.1.2-bin/bin/beeline -u jdbc:hive2://${EMR_HOST_NAME}:10000;AuthMech=6;delegationToken=${JWT_TOKEN}\" >> /etc/profile.d/rstudio.sh\"" >> /etc/cont-init.d/bootstrap_container && \
