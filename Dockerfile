...
ENV USER_PERSISTED_VARS AWS_CONTAINER_CREDENTIALS_RELATIVE_URI AWS_DEFAULT_REGION AWS_EXECUTION_ENV AWS_REGION \
    ECS_CONTAINER_METADATA_URI S3_BUCKET USER KMS_HOME KMS_SHARED EMR_HOST_NAME HTTP_PROXY HTTPS_PROXY NO_PROXY http_proxy https_proxy no_proxy
...
RUN echo "#!/usr/bin/with-contenv bash" > /etc/cont-init.d/bootstrap_container && \
    ...
    echo "ln -s /mnt/s3fs /home/\${USER}/s3" >> /etc/cont-init.d/userconf
